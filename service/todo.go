...
func (s *TODOService) UpdateTODO(ctx context.Context, id int64, subject, description string) (*model.TODO, error) {
	...
	// 変更された行数を取得
	//RoesAffectedはSQLクエリがデータベース内で何行に影響を与えたかを確認するために使われる。
	//int64 型の値で、影響を受けた行数を返す。
	rowsAffected, err := result.RowsAffected()

	if err != nil {
		log.Println("RowsAffectedでエラー発生:", err)
		return nil, err
	}

	// 変更された行が0件の場合、ErrNotFoundを返す
	if rowsAffected == 0 {
		log.Println("RowsAffectedが0件")
		return nil, &model.ErrNotFound{}
	}

	// 挿入されたレコードの ID を取得（ExecContextメソッドの戻り値から保存したTODOのIDを取得）
	id, err = result.LastInsertId() //:=は新しい変数を定義するために使用する。=は既存の値に対して使用する。
